    ...
    <div className="space-y-2">
      <Popover.Root open={open} onOpenChange={setOpen}>
        <Popover.Trigger asChild>
          ...
          >
            <span className="truncate">{selected.length > 0 ? `${selected.length} selecionado${selected.length > 1 ? 's' : ''}` : placeholder}</span>
            ...
            <Command className="w-full">
              ...
              <Command.List className="max-h-60 overflow-y-auto p-1">
                ...
                {options.map(o => (
                  ...
                  >
                    <Check size={14} className={cn('flex-shrink-0', value.includes(o.value) ? 'opacity-100 text-primary' : 'opacity-0')} />
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {selected.map(o => (
            <span key={o.value} className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-secondary text-xs text-foreground">
              {o.label}
              <button type="button" onClick={() => remove(o.value)} className="text-muted-foreground hover:text-red-400 transition">
                <X size={12} />
              </button>
            </span>
          ))}
